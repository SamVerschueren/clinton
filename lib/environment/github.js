...
class GitHubEnvironment {
	...
	constructor(opts) {
		this._opts = opts;
		this.fs = new GitHubFileSystem(this._opts);
	}

	load(repository) {
		this._opts.repository = repository;

		// Load the repository data
		return ghGot(`repos/${repository}`, {token: this._opts.token})
			.then(res => {
				const data = res.body;
				...
				return ghGot(`repos/${repository}/branches/${branch}`, {token: this._opts.token});
			})
			.then(res => {
				...
				return ghGot(`repos/${repository}/git/trees/${res.body.commit.sha}?recursive=1`, {token: this._opts.token});
			})
			.then(res => {
